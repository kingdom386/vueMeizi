<template>
  <div class="welfare-wrapper" v-infinite-scroll="loadMore" infinite-scroll-disabled="busy" infinite-scroll-distance="10">
    <div class="welfare-center">
      <figure v-show="leftData.length > 0" v-for="(data, i) in leftData" @click="selectDetails(data.createdAt)" :key="i">
        <v-img :imgUrl="data.url"></v-img>
      </figure>
    </div>
    <div class="welfare-center">
      <figure v-show="rightData.length > 0" v-for="(data, i) in rightData" @click="selectDetails(data.createdAt)" :key="i">
        <v-img :imgUrl="data.url"></v-img>
      </figure>
    </div>
    ...
    methods: {
      loadTop() {
        ...
        this.$http.get(`https://gank.io/api/data/福利/10/${this.page}`).then((response) => {
          ...
          this.$nextTick(() => {
            this.$store.commit('UPDATE_LOADING', false); // 修改加载状态
          });
        });
      },
      loadMore() {
